...
import { ProjectRequestDto, ProjectResponseDto } from '../../../../core/models/project.model';
...
export class ProjectCreateModalComponent implements OnInit {
	...
	loadManagers() {
		this.userService.getUsers(0, 10, '', 'ALL', 'true').subscribe({
			...
			error: (_err: HttpErrorResponse) =>
				this.toastService.show('Error al cargar responsables', 'error'),
		});
	}
	...
	onSubmit() {
		...
		const requestDto = Object.fromEntries(
			Object.entries(rawData).map(([key, value]) => [
				key,
				typeof value === 'string' ? value.trim().replace(/\s+/g, ' ') : value,
			]),
		) as unknown as ProjectRequestDto;
