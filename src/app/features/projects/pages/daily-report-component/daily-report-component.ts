...
export class DailyReportComponent implements OnInit {
	...
	buildResourcesForm() {
		...
		this.selectedItemInfo.apuDetails.forEach((apu) => {
			...
			const rowGroup = this.fb.group({
				...
				realQuantity: [
					{ value: 0, disabled: isMaterial },
					[Validators.required, Validators.min(0)],
				], // El ingeniero digita esto si no es material
			});
				...
				this.inventoryService
					...
					.subscribe({
						...
						error: (err: HttpErrorResponse) => {
							this.toastService.showApiError(err, 'Error fetching consumed quantity');
						},
					});
			}
		});
		...
		this.usedResourcesFormArray.controls.forEach((rowGroup) => {
			if (rowGroup.get('resourceType')?.value === ResourceType.MATERIAL) {
				this.inventoryService
					.getConsumedQuantity(
						Number(this.selectedItemInfo?.id),
						Number(rowGroup.get('resourceId')?.value),
						date,
					)
					.subscribe({
						next: (qty) => {
	...
	loadProjectItems() {
		this.itemService.getItems(this.projectId).subscribe({
			next: (data) => {
				this.executableItems = data.filter(
					(item) =>
						item.totalQuantity !== null &&
						item.totalQuantity > 0 &&
						item.apuDetails &&
						item.apuDetails.length > 0,
				);
			},
		});
	}
	...
	onSubmit(): void {
		...
		if (dto.executedQuantity > maxAllowed) {
			this.toastService.show(
				`El metrado reportado excede el saldo disponible (${maxAllowed} ${this.selectedItemInfo.unit}).`,
				`warning`,
			);
			return;
		}
